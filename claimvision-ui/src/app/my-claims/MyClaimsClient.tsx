  ...
  useEffect(() => {
    const fetchClaims = async () => {
      ...
      try {
        ...
        const apiUrl = process.env.NEXT_PUBLIC_API_GATEWAY as string | undefined;

        if (!apiUrl) {
          throw new Error('NEXT_PUBLIC_API_GATEWAY is not configured');
        }
          ...
          {claims.map((claim) => (
            ...
            >
              <div className="p-4 border-b">
                ...
                <p className="text-sm text-gray-500 mt-1">Created: {formatDate(claim.created_at)}</p>
              </div>
              <div className="p-4 bg-gray-50">
                <p className="text-sm text-gray-700 line-clamp-2">{claim.description || 'No description provided'}</p>
                <div className="mt-3 text-blue-600 text-sm font-medium">View Claim →</div>
              </div>
            </div>
          ))}
        </div>
