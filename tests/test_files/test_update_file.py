...
def test_update_file_success(mock_dynamodb, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PATCH", path_params={"id": "file-1"}, body=test_update_payload)
...
def test_update_file_not_found(mock_dynamodb, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PATCH", path_params={"id": "file-99"}, body=test_update_payload)
    mock_table = mock_dynamodb.return_value
    mock_table.get_item.return_value = {}  # ✅ Simulate file not found
...
def test_update_file_unauthorized(mock_dynamodb, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PATCH", path_params={"id": "file-1"}, body=test_update_payload, auth_user="user-999")
    mock_table = mock_dynamodb.return_value
...
def test_update_file_invalid_payload(mock_dynamodb, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PATCH", path_params={"id": "file-1"}, body=invalid_payload)
    mock_table = mock_dynamodb.return_value
