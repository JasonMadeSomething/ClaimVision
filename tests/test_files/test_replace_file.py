...
def test_replace_file_not_found(mock_dynamodb, mock_s3, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PUT", path_params={"id": "file-99"}, body=test_replace_payload, auth_user="user-123")
    mock_table = mock_dynamodb.return_value
...
def test_replace_file_unauthorized(mock_dynamodb, mock_s3, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PUT", path_params={"id": "file-1"}, auth_user="user-999", body=test_replace_payload)
    mock_table = mock_dynamodb.return_value
    mock_table.get_item.return_value = {"Item": {"id": "file-1", "user_id": "user-123"}}  # File exists, but wrong user
...
def test_replace_file_invalid_format(mock_dynamodb, mock_s3, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PUT", path_params={"id": "file-1"}, body=invalid_payload, auth_user="user-123")
...
def test_replace_file_empty_payload(mock_dynamodb, mock_s3, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PUT", path_params={"id": "file-1"}, body="{}", auth_user="user-123")
...
def test_replace_file_no_auth(mock_dynamodb, mock_s3, api_gateway_event):
    ...
    event = api_gateway_event(http_method="PUT", path_params={"id": "file-1"}, auth_user=None, body=test_replace_payload)
